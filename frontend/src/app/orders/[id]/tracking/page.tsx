...
export default function OrderTracking() {
  ...
  const [customerPos, setCustomerPos] = useState<[number, number]>([
    28.6139, 77.209,
  ]); // Default to Delhi until geolocation loads
    ...
    const fetchOrder = async () => {
      try {
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:4000"}/api/orders/${id}`,
          {
            headers: {
        ...
        if (res.data.deliveryCoords?.coordinates) {
          ...
          setDeliveryPos([
            res.data.deliveryCoords.coordinates[1],
            res.data.deliveryCoords.coordinates[0],
          ]);
        }
      } catch (err) {
  ...
  return (
    <div className="pt-16 min-h-screen bg-mesh flex flex-col lg:flex-row overflow-hidden">
      ...
      <div className="w-full lg:w-[400px] bg-white/80 backdrop-blur-xl border-r border-outline-variant/10 p-6 flex flex-col z-10 shadow-xl">
        <button
          onClick={() => router.back()}
          className="flex items-center gap-2 text-on-surface-variant hover:text-primary transition-colors mb-8 font-bold text-sm"
        >
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar space-y-6">
          <div className="pb-6 border-b border-outline-variant/10">
            <p className="text-[10px] font-black font-label text-primary uppercase tracking-widest mb-1">
              Live Tracking
            </p>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="font-bold text-on-surface">
                    {(order.deliveryBoyId as any)?.name ||
                      "Assigning Partner..."}
                  </p>
                </div>
              </div>
