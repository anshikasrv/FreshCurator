    ...
    return (
      <main className="pt-24 pb-24 max-w-4xl mx-auto px-6">
        <div className="animate-pulse space-y-6">
          {[1, 2, 3].map(i => (
            <div key={i} className="bg-surface-container-lowest rounded-2xl p-6 h-40" />
          ))}
        </div>
      </main>
    );
  ...
  return (
    <main className="pt-24 pb-24 max-w-4xl mx-auto px-6">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-4xl font-extrabold font-headline text-on-surface tracking-tight">My Orders</h1>
          <p className="text-on-surface-variant font-body mt-1">Track all your FreshCurator deliveries</p>
        </div>
        <Link href="/" className="flex items-center gap-2 bg-primary text-on-primary px-5 py-2.5 rounded-xl font-bold font-headline text-sm hover:scale-[1.02] transition-transform">
      ...
      {orders.length === 0 ? (
        <div className="text-center py-20">
          <span className="material-symbols-outlined text-6xl text-on-surface-variant mb-4 block">receipt_long</span>
          <h2 className="text-2xl font-bold font-headline mb-2">No orders yet</h2>
          <p className="text-on-surface-variant mb-8">Your order history will appear here after your first purchase.</p>
          <Link href="/" className="inline-flex items-center gap-2 bg-gradient-to-br from-primary to-primary-container text-on-primary-container px-8 py-4 rounded-xl font-bold font-headline hover:scale-[1.02] transition-transform">
    ...
    >
      <div className="bg-surface-container-lowest rounded-2xl shadow-sm border border-outline-variant/10 overflow-hidden group-hover:border-primary/50 group-hover:shadow-md transition-all">
        ...
        <div className="px-6 py-4 border-b border-outline-variant/10">
          <div className="flex items-center gap-0">
            {["Placed", "Accepted", "Out for Delivery", "Delivered"].map(
              (s, i, arr) => {
                const statuses = ["Placed", "Accepted", "Out for Delivery", "Delivered"];
                const currentIdx = statuses.indexOf(order.status);
                const stepIdx = statuses.indexOf(s);
        ...
        <div className="px-6 py-4">
          ...
          <div className="mt-4 text-center sm:text-right">
            <span className="text-primary text-xs font-bold flex items-center justify-end gap-1">
              View details & track <span className="material-symbols-outlined text-xs">arrow_forward</span>
            </span>
          </div>
        </div>
      </div>
