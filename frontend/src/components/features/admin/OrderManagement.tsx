...
import { fetchAllOrders, fetchAllUsers, updateOrderStatus, setAuthToken, Order, UserRecord } from '@/lib/api'; // 2. Import setAuthToken
import { io } from 'socket.io-client';
...
export default function OrderManagement() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setOrders(Array.isArray(o) ? o : (o as any).orders || (o as any).data || []);
      setUsers(Array.isArray(u) ? u : (u as any).users || (u as any).data || []);
    } catch (err) {
  ...
  if (loading) return (
    <div className="space-y-4">
        <p className="text-sm text-on-surface-variant animate-pulse">Fetching fresh orders...</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <p className="text-xs font-bold text-on-surface-variant uppercase">Total Records: {orders.length}</p>
        <button onClick={loadData} className="text-xs text-primary underline">Refresh Data</button>
      </div>
                    ...
                    <tbody>
                        {filtered.map(o => (
                             <tr key={o._id} className="border-t border-outline-variant/10 hover:bg-surface-container-low/30 transition-colors">
                                ...
                                <td className="px-6 py-4">
                                    <p className="font-bold font-headline text-sm text-left">#{o._id.slice(-8).toUpperCase()}</p>
                                    <p className="text-xs text-on-surface-variant mt-0.5 text-left">{new Date(o.createdAt).toLocaleDateString()}</p>
                                </td>
                                <td className="px-6 py-4 text-sm text-left">{typeof o.userId === 'object' ? o.userId.name : 'Customer'}</td>
                                <td className="px-6 py-4 text-sm text-on-surface-variant max-w-[150px] truncate">{o.deliveryAddress || '—'}</td>
                                <td className="px-6 py-4 font-extrabold text-primary">₹{o.totalAmount.toFixed(2)}</td>
